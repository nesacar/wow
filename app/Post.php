...
class Post extends Model
{
    ...
    protected $fillable = ['id', 'user_id', 'category_id', 'town_id', 'title', 'slug', 'short', 'body', 'image', 'widget_image', 'widget_text', 'link', 'views', 'publish', 'slider', 'widget', 'map', 'publish_at'];
...
//        self::cropImage($post->sliderImage, 480, 250);
    ...
    public static function getWidget($limit){
        return self::select('posts.id', 'posts.title', 'posts.slug','posts.widget_image', 'posts.widget_text', 'posts.widget', 'posts.short', 'posts.link', 'posts.publish_at', 'categories.slug as category')
            ->join('categories', 'posts.category_id', '=', 'categories.id')
