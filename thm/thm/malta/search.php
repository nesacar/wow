...
<section class="col-md-13 article">
  <h1>Search results: <span style='color: #FFF'><?= @$searchTerm ?></span></h1>
  <?php if(isset($noResult)) { ?>
    <p>Search produced no results. Look at our most read topics:</p>
  <?php } ?>
    ...
    ?>
      <article class="<?= $artclass ?>">
        <a href="<?= $link ?>"><?= $img ?></a>
		    <?php if($showH2) { ?><span class="grey"><?= Common::datum_en($r['published']) ?></span>&nbsp;<a href="<?= @$catLinks[$r['cat']] ?>"><?= @$catNames[$r['cat']] ?></a><?php } ?>
